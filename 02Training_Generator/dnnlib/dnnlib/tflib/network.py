...
    充当参数化网络构建功能的便捷包装，提供多种实用方法并方便地访问输入/输出/权重。

    可以出于长期存档的目的安全地对网络对象进行封存（pickle）和解封（unpickle）。只要在无副作用或不特定于应用程序的独立Python模块中定义了基础网络构建功能，pickle就可以可靠地工作。

    参数：
        name：网络名称。用于选择TensorFlow名称和变量作用域。
        func_name：基础网络构造功能或顶级函数对象的全限定名称。
        static_kwargs：要传递给网络构造函数的关键字参数。

    属性：
        name：用户指定的名称，如果为None，则默认为构建函数名称。
        scope：唯一的TensorFlow范围，包含从用户指定的名称派生的模板图和变量。
        static_kwargs：参数传递给用户提供的构建函数。
        components：子网的容器。传递给构建函数，并在两次调用之间保留。
        num_inputs：输入张量的数量。
        num_outputs：输出张量的数量。
        input_shapes：输入张量形状（NC或NCHW），包括小批量尺寸。
        output_shapes：输出张量形状（NC或NCHW），包括小批量尺寸。
        input_shape：input_shapes[0]的简写。
        output_shape：output_shapes[0]的简写。
        input_templates：在模板图中输入占位符。
        output_templates：模板图中的输出张量。
        input_names：每个输入的名称字符串。
        output_names：每个输出的名称字符串。
        own_vars：此网络定义的变量（local_name => var），不包括子网。
        vars：所有变量（local_name => var）。
        trainingable：所有可训练变量（local_name => var）。
        var_global_to_local：从变量全局名称到本地名称的映射。
    """
