...
def convert_images_to_uint8(images, drange=[-1,1], nchw_to_nhwc=False, shrink=1):
    ...
    if nchw_to_nhwc:
        images = tf.transpose(images, [0, 2, 3, 1])     #(?, 32, 32, 1)
    scale = 255 / (drange[1] - drange[0])       #127.5
    images = images * scale + (0.5 - drange[0] * scale)     #float32
    return tf.saturate_cast(images, tf.uint8)
    # return tf.saturate_cast(images, tf.float32)
    #tf.saturate_cast此处为溢出保护，此处相当于在剪切数值为0-255
